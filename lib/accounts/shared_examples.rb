shared_examples_for "it belongs to an account"  do
  ...
  it "should return account on #account" do
    expect(PadmaAccount).to receive(:find).with(object.account_name).and_return(PadmaAccount.new(account_name: object.account_name))
...
shared_examples_for "it belongs to a user"  do
  ...
  it "should return user on #user" do
    expect(PadmaUser).to receive(:find).with(object.username).and_return(PadmaUser.new(username: object.username))
